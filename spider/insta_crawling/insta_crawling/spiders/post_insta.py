...
class Post_Spider(scrapy.Spider):
    ...
    center_id = '1437038730'

    def start_requests(self):
        start_urls = 'https://www.instagram.com/graphql/query/?query_hash=bfa387b2992c3a52dcbe447467b4b771&variables=%7B%22id%22%3A%22{}%22%2C%22first%22%3A24%7D'.format(Post_Spider.center_id)
    ...
    def get_post(self, response):
        ...
        for edge in edges: # edge 안에서 가져와야 할 정보 추출
            ...
            #### 마지막 이미지 왜 안가져오지...????? #### -->> 4번째 node(총 사진 9장+영상1개=10갠데 마지막 사진 1개를 안가져와,,)
            media_url = []
            view_count = []
            try:
                ...
                # 중복되는 코드 줄이면 좋을듯
                    ...
                    for sidecar_edge in sidecar_edges:
                        if sidecar_edge['node']['__typename'] == 'GraphImage': # 이미지일때
                            display_url = sidecar_edge['node']['display_url']
                            media_url.append(display_url)
                        elif sidecar_edge['node']['__typename'] == 'GraphVideo': # 동영상일때
                            video_url = sidecar_edge['node']['video_url']
                            media_url.append(video_url)
                            view_count = sidecar_edge['node']['video_view_count']
        ...
        if end_cursor:
            time.sleep(1)
            next_page = 'https://www.instagram.com/graphql/query/?query_hash=bfa387b2992c3a52dcbe447467b4b771&variables=%7B%22id%22%3A%22{}%22%2C%22first%22%3A13%2C%22after%22%3A%22{}%3D%3D%22%7D'.format(Post_Spider.center_id, end_cursor[:-2])
